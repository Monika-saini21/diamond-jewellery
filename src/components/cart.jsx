...
function Cart() {
  ...
  let totalItem = cartItem?.reduce((sum, item) => sum + (item.quantity * item?.product.price), 0);
  ...
  function updateQuantity(data, change) {
    ...
    setCartQuantity(cartQuantity + change);
  }

  return (
    <>
      <h1 className="text-4xl ml-140 mt-50 text-yellow-600 border-b-3 h-30 w-33">Shopping Cart</h1>
      <div className='flex flex-col items-center'>
        {cartItem.length > 0 ? (
          cartItem.map(item =>
            item.quantity > 0 && (
              <div key={item.product.id} className='border-2 border-stone-200 w-280 h-54 rounded-3xl flex mb-5 items-center mt-5'>
                <img className='w-47 h-47 pl-4' src={item?.product?.image} alt={item?.product?.title} />
                <div className='w-150 h-45 pl-5'>
                  <p className='text-lg'>{item?.product?.title}</p>
                  <button onClick={() => { removeCartItem(item); setCartQuantity(0); }}>
                    <img
                  ...
                  <div className='flex gap-2'>
                    ...
                    <li className="flex items-center">
                      <button className="mr-1" onClick={() => updateQuantity(item, -1)}>
                        <img className='w-5 h-5 border-2 border-amber-400 p-0.5 rounded-sm'
                          src="https://cdn-icons-png.flaticon.com/128/10777/10777750.png" alt="Decrease" />
                      </button>
                      <p>{item?.quantity}</p>
                      <button className="ml-1" onClick={() => updateQuantity(item, 1)}>
                        <img className='w-5 h-5 rounded-sm' src={plus} alt="Increase" />
                      </button>
                    </li>
                  </div>
                  <p className='text-yellow-500 cursor-pointer'>Free Delivery</p>
                </div>
              </div>
            )
          )
          ...
          <div>
            <img className='w-100 mt-1' src='https://www.giva.co/cdn/shop/t/95/assets/empty-cart.jpg?v=2867468997513044031699807510' alt="Empty cart" />
